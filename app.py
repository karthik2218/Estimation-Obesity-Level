...
db=mysql.connector.connect(host='localhost', user='root',password='',port=3306,database='obesity')
cur=db.cursor()
app=Flask(__name__)
...
def Register():
    if request.method=="POST":
        ...
        if Password==confirmpassword:
            sql="select * from user where (Name='%s' and Password='%s') or (Email='%s' and Password='%s')"%(Name,Password,Email,Password)
            cur.execute(sql)
            data=cur.fetchall()
            db.commit()
            if data==[]:
                sql="insert into user(Name,Email,Contact,Age,Address,Password)values(%s,%s,%s,%s,%s,%s)"
